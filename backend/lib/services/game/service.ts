...
export const GameService = (): IGameService => ({
    processMessage: async (message: string, gameState: GameState) => {
        ...
        const newHonchoUserMessage = await honcho.apps.users.sessions.messages.create(gameState.honchoDefendant.appId, gameState.honchoDefendant.userId, gameState.honchoDefendant.sessionId, {
            content: message,
            is_user: true
        })
        ...
        const newHonchoAIMessage = await honcho.apps.users.sessions.messages.create(appId, userId, sessionId, { content: aiResponse.text, is_user: false })
